...
def extract_frames(video_path, frames_dir, overwrite=True, start=0, end=-1, every=1):
    """
    ...
    while frame < end:  # lets loop through the frames until the end
        ...
        if frame % every == 0:  # if this is a frame we want to write out based on the 'every' argument
            ...
            frame_path= os.path.join(frames_dir, video_filename)
...
if __name__=="__main__":
    ...
    video_path="../1.mov"
    # out_image_dolder_path=input("解析后的图片文件夹路径：")
    out_image_dolder_path="../frame"
    # frame_frequency=int(input("每多少帧读解析一张图片："))
    # frame_frequency=1
